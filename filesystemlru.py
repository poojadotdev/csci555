...
def benchmark_fuse_filesystem(mountpoint, num_files=15, reads_per_file=5, writes_per_file=2):
    ...
    Args:
        ...
        num_files (int): Number of files to create (default: 15, exceeds cache size 10).
        reads_per_file (int): Number of times to read each file.
        writes_per_file (int): Number of times to write to each file.
    """
    # Ensure mountpoint exists and is accessible
    ...
    # Step 1: Create and write to files
    start_time = time.time()
    for i in range(num_files):
        file_path = os.path.join(mountpoint, f"file{i}.txt")
        for _ in range(writes_per_file):
            with open(file_path, 'ab') as f:  # Append mode to simulate multiple writes
    ...
    for i in range(num_files):
        ...
        for _ in range(reads_per_file):
            ...
            # Optional: Print content to verify (comment out for pure benchmarking)
    ...
    # Give the FUSE filesystem time to process shutdown
    time.sleep(2)
