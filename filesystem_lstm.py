...
def shutdown_monitor(fs, mountpoint):
    ...
    while not fs.shutdown_flag.is_set():
        if os.path.exists("shutdown"):
            ...
            # Use access_count keys as the base set of all files
            all_paths = set(stats['access_count'].keys())
            for stat_type in ['access_count', 'cache_hits', 'cache_misses']:
                print(f"{stat_type}:")
                if all_paths:
                    for path in sorted(all_paths):  # Sort for consistent output
                        if stat_type == 'cache_misses':
                            # Calculate misses: accesses - hits (for files with data ops)
                            accesses = stats['access_count'].get(path, 0)
                            hits = stats['cache_hits'].get(path, 0)
                            misses = max(0, accesses - hits) if path in fs.data else 0
                            print(f"  {path}: {misses}")
                        else:
                            count = stats[stat_type].get(path, 0)
                            print(f"  {path}: {count}")
                else:
                    print("  (none)")
